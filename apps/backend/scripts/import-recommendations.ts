...
/** Single-quote a value for inlining into emitted SQL. */
function lit(value: string): string {
  return `'${value.replace(/'/g, "''")}'`;
}

/**
 * The same apply, as a file psql can run where this script cannot reach the
 * database.
 *
...
function buildSql(restaurantId: string, categories: SourceCategory[]): string {
  const rid = lit(restaurantId);
  const out: string[] = [
    "-- Generated by scripts/import-recommendations.ts --emit-sql",
    "-- Apply with:  psql -v ON_ERROR_STOP=1 -f <this file>",
    "--",
    "-- One transaction. Every ranking for the venue is cleared first, so this",
    "-- file is the complete statement of what gets recommended — a dish the",
    "-- owner dropped stops being pushed in the same breath. A dish name that",
    "-- matches nothing aborts the whole file rather than applying in part.",
    "",
    "BEGIN;",
    "",
    `UPDATE menu_items`,
  ...
  for (const section of categories) {
    ...
    for (const item of section.items) {
      ...
      const name = lit(item.name);
      const qualified = lit(`${item.name} (${section.category})`);
      const cat = lit(section.category);
      out.push(
        "DO $rec$",
        "DECLARE matched text;",
        "BEGIN",
        "  UPDATE menu_items",
        `     SET recommend_rank = ${item.priority}, is_signature = ${isSignature}, is_quick_bite = ${isQuick}`,
        "   WHERE id = (",
        "     SELECT id FROM menu_items",
        `      WHERE restaurant_id = ${rid}`,
        "        AND category_id = (SELECT id FROM menu_categories",
        `                            WHERE restaurant_id = ${rid} AND LOWER(name) = LOWER(${cat}))`,
        "        AND (",
        `          LOWER(name) = LOWER(${name})`,
        `          OR LOWER(name) = LOWER(${qualified})`,
        `          OR LOWER(${name}) LIKE '%' || LOWER(name) || '%'`,
        "        )",
        "      ORDER BY",
        `        CASE WHEN LOWER(name) = LOWER(${name}) THEN 0`,
        `             WHEN LOWER(name) = LOWER(${qualified}) THEN 1`,
        "             ELSE 2 END,",
        "        LENGTH(name) DESC",
        "      LIMIT 1",
        "   )",
        "  RETURNING name INTO matched;",
        "  IF matched IS NULL THEN",
        `    RAISE EXCEPTION 'No menu item matched % in section % for this venue. Nothing written.', ${name}, ${cat};`,
        "  END IF;",
        `  RAISE NOTICE '% #%: %', ${cat}, ${item.priority}, matched;`,
        "END",
        "$rec$;",
        ""
      );
...
function parseArgs(): { restaurantId: string; file: string; dryRun: boolean; emitSql: string | null } {
  ...
  if (!restaurantId || !file) {
    console.error(
      "Usage: tsx scripts/import-recommendations.ts --restaurant-id <uuid> --file <ranked.json> [--dry-run] [--emit-sql <out.sql>]"
    );
    process.exit(2);
  }
...
async function run(): Promise<void> {
  const { restaurantId, file, dryRun, emitSql } = parseArgs();
  ...
  // Emitting SQL deliberately touches no database — that is the whole point of
  // the flag, since the host that runs the file is not the host that has this
  // script. Import the pool only on the live path.
  if (emitSql) {
    writeFileSync(emitSql, buildSql(restaurantId, categories));
    const count = categories.reduce((n, c) => n + c.items.length, 0);
    console.log(`Wrote ${emitSql} — ${count} ranking(s) across ${categories.length} section(s).`);
    console.log("Apply with: psql -v ON_ERROR_STOP=1 -f " + emitSql);
    console.log("Names are NOT verified here; the file raises and rolls back on any miss.");
    return;
  }
