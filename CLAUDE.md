...
| Database **schema** | migration job runs before services roll (staging); **the VM does NOT migrate on restart** — you run it by hand from the new image | ⚠️ partly |
...
| GCP project | `bp-voxtable-stg` | `bp-voxtable-prod` — Cloud Run `voxtable-prod-api`/`-worker`/`-migrate` and the `voxtable-prod-deployer` SA **exist** (since #228, 20 Aug); the api revision dies at boot on the `LEGAL_DOCUMENTS_MANIFEST_URL` gate, so it serves nothing |
...
| Phone number | `+61 468 203 234` (bound and answering) | `+61 468 202 846` — 🟢 **answers**: Biteperk workspace, agent `agent_b6b6488af08b82d80e8f4d270a` ("Mazcina Resto-Bar (production)"), venue row `44444444-…`. Four real calls reached it on 20 Aug. ⚠️ It still drops roughly one call in three at ~7.6 s — the AU1 trunk incident, open. **Read this line's Retell state with the VM's key only.** The repo's local `.env` holds the **legacy Algorythmos workspace** key; read with it, this number 404s and the correct agent "does not exist", which is exactly how an earlier entry here came to claim the number was absent — and how acting on that claim took the line down for two hours on 20 Aug. `npm run check:voice-lines` loads the right credentials per line from [`deploy/voice-lines.json`](deploy/voice-lines.json) and refuses to run on a key that cannot see the declared agent. The pilot line `+61 2 7501 1140` remains on the suspended Algorythmos Twilio account. See [`NUMBERS.md`](NUMBERS.md) §2. |
...
| Cal.com online bookings | One account, one API key, one rate limit shared by both environments; nothing at the vendor stops staging and production binding the same event type, so a staging test booking can write a real reservation | A separate webhook per environment; the `calcom_inbox_tenant_mismatch` tripwire (a staging push carries a restaurant UUID production does not have); the unmapped-event-type alarm. A second Cal.com team is the only real fix — see [`deploy/runbooks/calcom-integration.md`](deploy/runbooks/calcom-integration.md) |
...
**Cloud Run deploys migrate first**: `deploy-backend.yml` (rewritten 4 Aug 2026) executes the Cloud Run job `voxtable-stg-migrate` — the new api image with `APP_ENV=migration`, running `dist/db/migrate.js` — and only rolls the api/worker services after it succeeds, so new code never serves against an old schema. **Migrations must run as `voxtable_owner`, never `postgres`**: migration 029's `ALTER DEFAULT PRIVILEGES` attaches to the executing role, so the wrong role silently strips future tables of `voxtable_app` grants. (029 splits owner/runtime roles and adds the `BEFORE TRUNCATE` guard on `agreement_acceptances`; it is a guarded no-op where the roles don't exist, i.e. CI and dev.) ⚠️ **The VM path — production until the Phase 3 cutover — does NOT migrate on restart.** `Dockerfile.api` is `CMD ["node", "apps/backend/dist/server.js"]`; there is no migrate step anywhere in the image start. This file previously claimed otherwise and that claim caused a silent no-op on 20 Aug 2026: `docker compose exec api node …/migrate.js` runs the ALREADY-RUNNING container, so on the old image it only knows the migrations that image shipped with, finds nothing new, and exits cleanly having done nothing. **Migration files ship inside the image**, so the order is: `pull` → `run --rm api node apps/backend/dist/db/migrate.js` (the NEW image, app not started) → verify `schema_migrations` → `up -d`. That still gives schema-before-code.
...
- **Pulling images on the VM needs a per-user Docker credential helper**, not a token login and not an instance restart. `gcloud auth configure-docker australia-southeast1-docker.pkg.dev` **without sudo** (sudo writes root's config; compose runs as the deploying user). The VM's `devstorage.read_only` scope is sufficient for Artifact Registry reads — AR accepts storage scopes — so the "scopes block the registry, needs a stop/start" belief recorded from 3 Aug is wrong and cost four deferrals. That scope limit IS real for **writing** to GCS, which is why backups run off-VM; the two are different operations.
...
- **Promoting `main` targets Cloud Run production (`voxtable-prod-*`), and it fails at container startup — not at validation.** This line used to say `bp-voxtable-prod` was an empty shell with no deployer SA. Abhishek's "Release to Prod" (#228, 20 Aug) proved otherwise: run `32397027519` authenticated as `voxtable-prod-deployer@bp-voxtable-prod`, updated the Cloud Run job `voxtable-prod-migrate`, then `gcloud run services update voxtable-prod-api` failed with *"container failed to start and listen on PORT=3050"* — which is what `config/env.ts` does in production without `LEGAL_DOCUMENTS_MANIFEST_URL` (required since #218; see the legal-documents section). So the Cloud Run production target is real and half-provisioned; **what blocks it is the legal manifest, the same thing that blocks the VM** (its `.env` lacks the variable and the VM is frozen on `api:0.1.0` / migration 024). Production serving traffic is still the VM; a `main` merge does NOT reach it. ⚠️ **The "frozen on `api:0.1.0` / migration 024" claim above is stale — read off the box on 1 Sep 2026 it is `api:0.2.0` / `worker:0.2.0`, both healthy, at 36 migrations through `036_calcom_inbox_retry.sql`, and `LEGAL_DOCUMENTS_MANIFEST_URL` IS set in the VM's `.env`.** The repo is at 042, so the VM is six migrations behind — see `deploy/runbooks/mazcina-production-recommendations.md`. Still missing on the Cloud Run side: the `api.biteperk.com.au` → Cloud Run domain mapping, and the env var in the prod Terraform map. The unblock sequence is recorded in `deploy/retell-snapshots/20260819-prod-mazcina-post/README.md` (publish legal docs → set the var in VM `.env` + Terraform → promote → migrate from the new image → deploy).
