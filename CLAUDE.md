...
| Phone number | `+61 468 203 234` (bound and answering) | `+61 468 202 846` — 🔴 **not reachable: absent from the Retell workspace.** Verified 20 Aug 2026 (`list-phone-numbers`): the Biteperk workspace holds exactly one number, the pilot line `+61 2 7501 1140`, in webhook mode. Twilio's side is wired and the Mazcina agent `agent_3bedcbdd77017136e5b4ade412` passes every golden-config check — but nothing routes to it. It **was** imported on 19 Aug (a call reached it and dropped at 7,595 ms). **Why it went, answered 20 Aug:** a number lives in exactly one Retell workspace *account-wide*, and this one is held by a workspace neither BitePerk key can see — `import-phone-number` is refused with `Phone number already exists` while `get-phone-number` 404s in both our workspaces. Releasing it is a dashboard job; then `apply-line.mjs +61468202846 --apply`. The venue row and agent were **also** broken and are fixed: the row pointed at an agent id that did not exist. Run `npm run check:voice-lines` — it asserts the whole chain against [`deploy/voice-lines.json`](deploy/voice-lines.json) and names the layer that broke. The AI/recording disclosure was restored on all three production agents 20 Aug. The pilot line remains on the suspended Algorythmos Twilio account. See [`NUMBERS.md`](NUMBERS.md) §2. |
