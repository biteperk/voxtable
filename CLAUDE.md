...
**Known gaps in the new pipeline — check before relying on it.** The deploy half is unfinished, and these were verified on 1 Aug 2026:
- `GCP_VM_NAME` / `GCP_VM_ZONE` are unset in **both** GitHub environments. Staging deploys already fail on this (`Set GCP_VM_NAME in the GitHub environment`). **`production` is empty too**, so the first promotion to `main` will break production deploys unless they are set (`core-central-vm` / `us-central1-a`). Production currently still deploys via the older push-to-`main` workflow, which is why it works today.
- There is **no staging VM** — `core-central-vm` is the only instance.
- The image registry (`bp-shared-artifacts`) and the frontend artifact bucket (`voxtable-frontend-artifacts`) live outside the `vocotable-497209` project and are **not readable by Sam's account** — worth resolving for auditability and bus factor.
