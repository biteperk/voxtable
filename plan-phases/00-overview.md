...
## Product Surface
- Voice booking agent answers inbound calls through Vapi/Twilio.
...
- Telephony: Twilio Australian number or call forwarding.
...
- Store booking and call state in Postgres from day one; do not rely on Vapi, Twilio, or dashboard memory as the source of truth.
...
- `call_logs`: Vapi/Twilio IDs, call status, transcript, recording URL, latency, transfer outcome.
...
- Left sidebar navigation with restaurant identity and AI online state.
...
- AI Configuration for prompt/FAQ/transfer settings.
...
- Provider instability: Vapi, Twilio, ElevenLabs, and LLM providers must be isolated behind clear contracts.
